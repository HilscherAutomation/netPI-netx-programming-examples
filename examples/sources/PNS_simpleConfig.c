...
int main(int argc, char* argv[])
{
  ...
  while( (lRet = stat("/opt/cifx/plugins/netx-spm/config0",&buffer)) != 0) {
    ...
    if( system("dpkg -i ./driver/netx-docker-pi-drv-1.1.3.deb") == -1 ) {
      printf("Installing the driver failed. Check if driver package 'netx-docker-pi-1.1.3.deb' is located in folder './driver'(relative path to demo app).\n");
      return 0;
    }
  }
  ...
  while( (lRet = stat("/opt/cifx/deviceconfig/FW/channel0/R160D000.nxf",&buffer)) != 0) {
    ...
    if( system("dpkg -i ./firmwares/netx-docker-pi-pns-3.12.0.2.deb") == -1) {
      printf("Installing the firmware failed. Check if firmware 'netx-docker-pi-pns-3.12.0.2.deb' package is located in folder './firmwares'(relative path to demo app).\n");
      return 0;
    }
  }
