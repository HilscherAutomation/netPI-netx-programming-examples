 ...
 **************************************************************************************
   ...
   Filename:
    $Workfile: PLS_simpleConfig.c $
   Last Modification:
    $Author : AB$
    $Modtime: 27.03.19 09:40 $
    $Revision$
   ...
   Description:
     Linux I/O test application configuring netX as POWERLINK slave with 
   ...
   Changes:
     ...
     10        08.04.2019  PST      - example simplified
      ...
      4        16.07.2017  AB       - added POWERLINK slave PacketHandlerPLS.c example 
...
int32_t ChannelDemo()
{
  ...
  if(CIFX_NO_ERROR == lRet)
  {
    ...
    } else
    {
      ...
      printf("\nSending POWERLINK slave configuration request!\n");
      printf("\nConnect your POWERLINK master now to any of the two Industrial Ethernet ports.\n");
...
int main(int argc, char* argv[])
{
  ...
  printf("It configures the netX as POWERLINK slave in the length of 10 bytes input and 4 bytes output data.\n\n");
  ...
  /* check if the POWERLINK slave has been installed properly on the system, else install its package */
  while( (lRet = stat("/opt/cifx/deviceconfig/FW/channel0/R160K000.nxf",&buffer)) != 0) {
    printf("\nThe POWERLINK slave firmware has not been found installed.\n");
    printf("Installing the firmware now.\n");
    if( system("dpkg -i ./firmwares/netx-docker-pi-pls-3.3.0.0.deb") == -1) {
      printf("Installing the firmware failed. Check if firmware 'netx-docker-pi-pls-3.3.0.0.deb' package is located in folder './firmwares'(relative path to demo app).\n");
      return 0;
    }
  }
